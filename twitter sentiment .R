...
data_train <- data_train[,!names(data_train) %in% c("next")]
model = randomForest(as.factor(sentiment) ~ .,data=data_train, importance=TRUE, ntree=2000)

test.x = as.data.frame(test.x)
par_pred = predict(model, newdata = test.x, type="response")
