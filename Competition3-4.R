...
#-------------------------------------------------------#
training_set_reduced = training_set_reduced[1:979,]
attach(training_set_reduced)
# model fit with all 82 variables 
model= glm(sentiment~., data = training_set_reduced)
summary(model)

par_pred_model = predict(model, newdata = vars_test_reduced, type="response")
...
par_pred_resutl.s = predict(result.s, newdata = vars_test_reduced, type="response")

# Another model building procedure using multinom 
# multinom
library(nnet)
...
par_pred = predict(fit_multinom, vars_test_reduced, type="class")
...
# ----------------------------------------------------#
library(boot)
# We want to compare if our model result.s using the stepwise model selection is 
# better than the model with all the variables. Here below are the cross validation errors for these two models.
# for model 
cv.err=cv.glm(training_set_reduced, model)
cv.err$delta  #  0.6184062 0.6183478
# for result.s
cv.err=cv.glm(training_set_reduced, result.s)
cv.err$delta # 0.5649425 0.5649228


# By comparing the cross validation errors, we decide to use the model "result.s".

test = read.csv("test.csv")
table = data.frame(test$id,round(par_pred_resutl.s))
